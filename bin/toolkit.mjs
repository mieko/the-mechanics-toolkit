...
if (command === "inspect" && args.length === 1) {
  ...
  if (patchName !== "terminal-toggle") fail(`Unknown patch: ${patchName}`);
  if (!new Set(["check", "apply"]).has(action)) fail(`Unknown patch action: ${action}`);
  const patch = path.join(root, "patches/terminal-toggle/patch.mjs");
  ...
  fail(
    "usage:\n" +
      ...
      "  mechanics-toolkit patch terminal-toggle check|apply EXTRACTED_ASAR_ROOT"
  );
}
