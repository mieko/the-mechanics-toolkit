...
| Renderer turn window | Run its bounded-turn probe against the packed candidate, then switch into a long-lived real task that previously stalled. | The mounted UI projection keeps only the newest 1,500 complete turns, transport pagination and transcript export remain intact, and task switching no longer stalls. |
